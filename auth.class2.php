...
class ctrl_auth
{
    ...
    static function Authenticate($username, $password, $rememberMe = false, $isCookie = false, $sessionSecurity = false)
    {
        ...
        if ($row) {
            ...
            ctrl_auth::SetSession('extUsername', $row['ac_user_vc']);
			
			//extplorer session auth
    ...
    static function KillSession()
    {
		...
		//for extplorer clear session
        if (isset($_SESSION['extUsername'])) {
            unset($_SESSION['extUsername']);
        }
